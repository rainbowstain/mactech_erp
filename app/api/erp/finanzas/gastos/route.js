...
import { normalizeExpenseType, normalizePaymentMethod } from "@/lib/finance";
...
export async function POST(request) {
  ...
  const result = await query(
    ...
    [
      ...
      monto,
      normalizeExpenseType(body.tipo),
      body.metodo_pago ? normalizePaymentMethod(body.metodo_pago) : null,
    ]
  );
...
export async function DELETE(request) {
  ...
  const result = await query("update finanzas_gastos set estado = 0 where id = $1 returning *", [id]);
  if (!result.rows[0]) return NextResponse.json({ message: "Gasto no encontrado." }, { status: 404 });
  return NextResponse.json(result.rows[0]);
}
...
export async function PATCH(request) {
  ...
  const result = await query("update finanzas_gastos set estado = $1 where id = $2 returning *", [estado, id]);
  if (!result.rows[0]) return NextResponse.json({ message: "Gasto no encontrado." }, { status: 404 });
  return NextResponse.json(result.rows[0]);
}
