...
export async function getClients({ search = "" } = {}) {
  ...
  if (cleanSearch) {
    ...
    // Ademas del ILIKE literal, compara el RUT sin puntuacion: asi encuentra
    // al cliente aunque se busque "123456789" y este guardado "12.345.678-9".
    const runKey = rutSearchKey(cleanSearch);
    if (runKey && /\d/.test(runKey)) {
      params.push(`%${runKey}%`);
      where += ` or lower(regexp_replace(coalesce(run, ''), '[^0-9kK]', '', 'g')) like $${params.length}`;
    }
  }
  const result = await query(
